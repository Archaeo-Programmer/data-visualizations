...
df <- data.frame(
  ...
  dplyr::mutate(across(-month, ~ round(.x / rowSums(across(-month)) * 100)),
                moyr = forcats::fct_reorder(format(zoo::as.yearmon(month, format = "%B %Y"), "%B %Y"), month)) %>% 
  tidyr::pivot_longer(cols = -c(month, moyr), names_to = "asset_class", values_to = "allocation") %>% 
...
barchart <- df %>%
  ...
  coord_flip() +
  geom_label(aes(label = I(sprintf("%.1f%%", allocation)), hjust=0), fill = 'white') +
  scale_fill_manual(values = m.color) +
  scale_y_continuous(labels = function(x)  scales::percent(x, scale = 1)) +
  transition_states(moyr,
                    transition_length = 10,
                    state_length = 1) +
  labs(title = "Investment Portfolio Allocation Trends (2014-2023)",
       subtitle = '{closest_state}', fill = "asset_class", 
       caption = 'Source: AGB Investment Firm (simulated dataset)',
       x = "Asset Class",
       y = "Allocation (%)") +
  theme(legend.position="none",
        text = element_text(colour = "white"),
        axis.title.x = element_text(margin = unit(c(5, 5, 5, 5), "mm"),
                                    size=14,
                                    face="bold",
                                    colour = "white"),
        axis.title.y = element_text(margin = unit(c(5, 5, 5, 5), "mm"),
                                    size=14,
                                    face="bold",
                                    colour = "white"),
...
bar_sassy_perc_change <- shares.long %>%
  ...
  dplyr::mutate(wave_name = as.factor(format(as.yearmon(wave), "%B %Y")),
                perc = paste0(prop, "%")) %>%
  dplyr::group_by(segment) %>% 
  mutate(perc_change = prop - lag(prop),
         rn = row_number()) %>%
  ungroup %>% 
  dplyr::mutate(year = readr::parse_number(as.character(wave_name))) %>% 
  # Make a ggplot, but add frame=year: one image per year
  ggplot(., aes(x = forcats::fct_rev(segment), y = perc_change, fill = segment)) +
  #geom_bar(stat = 'identity') +
  geom_chicklet(radius = grid:: unit(3, "mm"), aes(group = segment), color = "transparent") +
  #theme_bw() +
  ...
  labs(title = "Global Warming’s Six Americas",
       subtitle = '{closest_state}', fill = "Segment", 
       caption = 'Source: Yale Program on Climate Change Communication;\nGeorge Mason University Center for Climate Change Communication') +
  xlab("") +
  ylab("Percent Change in U.S. Adult Population") +
  theme(legend.position="none",
        text = element_text(colour = "white"),
        axis.title.x = element_text(margin = unit(c(5, 10, 10, 10), "mm"),
                                    size=14,
                                    face="bold",
                                    colour = "white"),
        axis.text=element_text(size=12, colour = "white"),
        axis.title=element_text(size=14,face="bold", colour = "white"),
        plot.title = element_text(size=20, face="bold", colour = "white"),
        plot.caption = element_text(hjust = 0, colour = "white"))
